...
class WebUIChat_gpt4(View):
    ...
    def get(self,request):
        ...
        if not user.GPT_4_8K:
            return HttpResponse('<h1>你还没有开通GPT4的权限哦</h1>')
...
# claude-3-haiku 网页聊天框
class WebUIClaude3_Haiku(View):
    ...
    def get(self,request):
        ...
        return render(request,'webui/index_claude3_haiku.html',locals())
    

    @method_decorator(check_jwt)
    def post(self,request,*args,**kwargs):

        ANTHROPIC_AI_MODEL_NAME_CLAUDE3 = 'claude-3-haiku-20240307'    
        ...
        #print('inputmessage',inputmessage)
        #inputmessage.insert(0,selected_system_role_dict)
        system_prompt_template = selected_system_role_dict.get('content','you are a helpful Ai')
        print('system_prompt_template',system_prompt_template)
        #print('selected_system_role_dict',selected_system_role_dict)
        print('inputmessage2',inputmessage)
        ...
        # 内置定义函数
        def chat_stream_generator(MODEL_NAME=ANTHROPIC_AI_MODEL_NAME_CLAUDE3,
                                  inputmessage=inputmessage,
            ...
            try:
                ...
                with client.messages.stream(
                    model = ANTHROPIC_AI_MODEL_NAME_CLAUDE3,
                    system=system_prompt_template,
                    messages = inputmessage,
                    temperature = MODEL_TEMPERATURE,
                    max_tokens = MAX_TOKEN_RESPONSE,
        ...
        # 返回数据流给前端
        return StreamingHttpResponse(chat_stream_generator(
                                        MODEL_NAME = ANTHROPIC_AI_MODEL_NAME_CLAUDE3,
                                        inputmessage=inputmessage,
                                        MODEL_TEMPERATURE=selected_system_model_temperature,
                                        MAX_TOKEN_RESPONSE = selected_system_max_reponse_tokens,
                                        MODEL_TOP_P = selected_system_model_p,
    ...
    def delete(self,request,*args,**kwargs):
        ...
        try:
            if system_role_id and system_role_title:
                BlackBox.objects.create(user_id=user_id,RoleVoiceAttribution_id=system_role_id,diolog=inputmessage,GPT_model_name='claude-3-haiku')
            # 如果没有选择角色那就归类到无角色中
            else:
                BlackBox.objects.create(user_id=user_id,RoleVoiceAttribution_id=39,diolog=inputmessage,GPT_model_name='claude-3-haiku')
        except Exception as e:
            print(traceback.format_exc())
...
class WebUIClaude3_Sonnet(View):
    ...
    def get(self,request):
        ...
        return render(request,'webui/index_claude3_sonnet.html',locals())
    

    @method_decorator(check_jwt)
    def post(self,request,*args,**kwargs):

        ANTHROPIC_AI_MODEL_NAME_CLAUDE3 = 'claude-3-sonnet-20240229'    
    ...
    def delete(self,request,*args,**kwargs):
        ...
        try:
            if system_role_id and system_role_title:
                BlackBox.objects.create(user_id=user_id,RoleVoiceAttribution_id=system_role_id,diolog=inputmessage,GPT_model_name='claude-3-sonnet')
            # 如果没有选择角色那就归类到无角色中
            else:
                BlackBox.objects.create(user_id=user_id,RoleVoiceAttribution_id=39,diolog=inputmessage,GPT_model_name='claude-3-sonnet')
        except Exception as e:
            print(traceback.format_exc())
